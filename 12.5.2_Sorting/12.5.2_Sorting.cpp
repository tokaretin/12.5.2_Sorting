...
в консоль в порядке от большего к меньшему. По возможности используйте минимум дополнительной 
...
void bubbleSort(float array[], int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            // array[j] > array[j + 1] отсортировать элементы по возрастанию.
            // array[j] < array[j + 1] отсортировать элементы по убыванию
